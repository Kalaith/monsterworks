# Monsterworks Backend

Plain PHP backend for Monsterworks using WebHatchery bearer-token auth and guest sessions.
...
Run `database/migrations/001_create_players_and_saves.sql` against the `monsterworks` database. The migration uses `CREATE TABLE IF NOT EXISTS` and is safe to rerun for the baseline tables.
