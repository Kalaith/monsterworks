...
export class CreatureAIService {
   ...
   */
  static findNearestWork(creature: CreatureState, buildings: BuildingState[]): BuildingState | null {
    return buildings
      .filter(building => 
   ...
   */
  static findNearestStorage(creature: CreatureState, buildings: BuildingState[]): BuildingState | null {
    return buildings
    ...
    return creatures.map(creature => {
      ...
      if (creature.status === 'idle' && creature.energy > 50) {
        ...
        if (now - lastWorkSearch >= workSearchCooldown) {
          ...
          if (nearestBuilding) {
            console.log(`🎯 ${new Date().toLocaleTimeString()} Creature ${creature.id} found work at ${nearestBuilding.id}`);
   ...
   */
  static updateCreatureMovement(creatures: CreatureState[], buildings: BuildingState[], deltaTime: number): CreatureState[] {
    return creatures.map(creature => {
      ...
      if (distance < 5) {
        ...
        if (targetBuilding?.type === 'corpse_pile' && creature.carriedAmount > 0) {
          console.log(`📦 ${new Date().toLocaleTimeString()} Creature ${creature.id} dropping off resources`);
          newCreature.carriedAmount = 0;
          newCreature.status = 'idle';
          newCreature.targetBuilding = undefined;
        } else {
          console.log(`🔨 ${new Date().toLocaleTimeString()} Creature ${creature.id} starting work`);
  ...
  static updateCreatureWork(creatures: CreatureState[], buildings: BuildingState[]): { creatures: CreatureState[], buildings: BuildingState[] } {
    ...
    const updatedCreatures = creatures.map(creature => {
      ...
      if (creature.carriedAmount < creatureData.capacity && targetBuilding.production >= 1) {
        ...
        console.log(`📦 ${new Date().toLocaleTimeString()} Creature ${creature.id} collecting ${collectAmount}`);
